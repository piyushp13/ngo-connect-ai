      ...
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow p-6">
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            ...
            {ngos.map(ngo => (
              <div key={ngo._id} className="border rounded-lg p-4 flex items-start justify-between gap-4">
                <div>
                  <h3 className="font-semibold text-gray-800">{ngo.name}</h3>
                  <p className="text-sm text-gray-500">{ngo.email}</p>
                  <p className="text-sm text-gray-500">{ngo.location || 'Location not set'}</p>
                </div>
